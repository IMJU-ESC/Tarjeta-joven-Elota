...
export default function ActivarCuenta() {
  ...
  return (
    <main className="app-shell motion-enter min-h-screen overflow-hidden bg-[#080d18] px-5 py-8 text-white sm:grid sm:place-items-center">
      <div className="brand-orb absolute -left-24 top-24 h-80 w-80 rounded-full bg-orange-500/15 blur-3xl"></div>
      ...
      <section className="relative mx-auto grid w-full max-w-4xl overflow-hidden rounded-[2.4rem] border border-white/10 bg-white/[.07] shadow-2xl backdrop-blur-xl md:grid-cols-[.85fr_1.15fr]">
        <div className="relative hidden overflow-hidden bg-gradient-to-br from-orange-500 via-orange-600 to-pink-600 p-10 md:flex md:flex-col md:justify-between">
          ...
          <div className="relative">
            ...
            <p className="mt-4 text-sm font-semibold leading-6 text-orange-50/85">Crea una contraseña segura y comienza a usar Tarjeta Joven Elota.</p>
          </div>
        </div>
        ...
        <div className="p-6 sm:p-10 md:p-12">
          <div className="mb-8 flex items-center justify-between gap-4 md:hidden">
            ...
            <div className="rounded-full border border-orange-300/20 bg-orange-400/10 px-4 py-2 text-[9px] font-black uppercase tracking-[.18em] text-orange-200">Activación segura</div>
          </div>

          {status === "verificando" && (
            <div className="grid min-h-72 place-items-center text-center">
              <div><div className="mx-auto h-12 w-12 animate-spin rounded-full border-4 border-white/10 border-t-orange-400"></div><p className="mt-5 text-xs font-extrabold text-slate-300">Verificando tu invitación…</p></div>
            </div>
          )}

          {(status === "listo" || status === "guardando") && (
            <form onSubmit={guardar}>
              <p className="text-[9px] font-black uppercase tracking-[.24em] text-orange-300">Un último paso</p>
              ...
              <div className="mt-8 space-y-4">
                <label className="block"><span className="mb-2 block text-[10px] font-black uppercase tracking-widest text-slate-400">Nueva contraseña</span><input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full rounded-2xl border border-white/10 bg-white/10 px-5 py-4 text-sm font-bold text-white outline-none transition focus:border-orange-400 focus:ring-4 focus:ring-orange-400/10" placeholder="Mínimo 8 caracteres" autoComplete="new-password" required /></label>
                <div className="h-1.5 overflow-hidden rounded-full bg-white/10"><div className={`h-full rounded-full transition-all duration-500 ${fuerza >= 80 ? "bg-emerald-400" : fuerza >= 50 ? "bg-yellow-300" : "bg-orange-500"}`} style={{ width: `${fuerza}%` }}></div></div>
                <label className="block"><span className="mb-2 block text-[10px] font-black uppercase tracking-widest text-slate-400">Confirmar contraseña</span><input type="password" value={confirmacion} onChange={(e) => setConfirmacion(e.target.value)} className="w-full rounded-2xl border border-white/10 bg-white/10 px-5 py-4 text-sm font-bold text-white outline-none transition focus:border-orange-400 focus:ring-4 focus:ring-orange-400/10" placeholder="Escríbela nuevamente" autoComplete="new-password" required /></label>
              </div>
              {mensaje && <p className="mt-4 rounded-2xl bg-red-400/10 px-4 py-3 text-xs font-bold text-red-200">{mensaje}</p>}
              <button type="submit" disabled={status === "guardando"} className="shine-sweep relative mt-7 w-full overflow-hidden rounded-2xl bg-gradient-to-r from-orange-500 to-pink-500 py-4 text-[11px] font-black uppercase tracking-widest shadow-xl shadow-orange-950/30 transition active:scale-[.98] disabled:opacity-60">{status === "guardando" ? "Activando…" : "Activar mi cuenta"}</button>
