...
export async function sendSystemMail(input: {
  ...
  const content = activation
    ? `<p>Hola <strong>${name}</strong>,</p>
       ...
       <p style="text-align:center;margin:30px 0"><a href="${actionLink}" style="display:inline-block;background:linear-gradient(135deg,#f58220,#f70476);color:#fff;text-decoration:none;padding:16px 26px;border-radius:16px;font-weight:900;box-shadow:0 10px 24px rgba(231,102,19,.22)">Crear mi contraseña →</a></p>
  ...
  await transporter.sendMail({
    ...
    html: `<div style="margin:0;padding:26px 12px;background:#f3f5f9;font-family:Aptos,'Segoe UI',Arial,sans-serif;color:#172033">
      <div style="max-width:620px;margin:auto;overflow:hidden;background:#ffffff;border:1px solid #fed7aa;border-radius:26px;box-shadow:0 18px 50px rgba(15,23,42,.09)">
        <div style="padding:22px 26px;background:#080d18;text-align:center">
          ...
          <p style="margin:14px 0 0;color:#fdba74;font-size:11px;font-weight:900;letter-spacing:2px">TARJETA JOVEN ELOTA</p>
        </div>
        ...
        <div style="height:7px;background:linear-gradient(90deg,#f58220,#f4c425,#25883a,#24b5d6,#f70476)"></div>
      </div>
    </div>`,
  });
}
